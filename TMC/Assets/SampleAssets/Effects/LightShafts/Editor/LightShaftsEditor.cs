...
internal class LightShaftsEditor : Editor
{
    private SerializedObject so;

    private SerializedProperty cameras;
    private SerializedProperty shadowmapMode;
    private SerializedProperty size;
    private SerializedProperty near;
    private SerializedProperty far;
    private SerializedProperty cullingMask;
    private SerializedProperty colorFilterMask;
    private SerializedProperty brightness;
    private SerializedProperty brightnessColored;
    private SerializedProperty extinction;
    private SerializedProperty minDistFromCamera;
    private SerializedProperty shadowmapRes;
    private SerializedProperty colored;
    private SerializedProperty colorBalance;
    private SerializedProperty epipolarLines;
    private SerializedProperty epipolarSamples;
    private SerializedProperty depthThreshold;
    private SerializedProperty interpolationStep;
    private SerializedProperty showSamples;
    private SerializedProperty backgroundFade;
    private SerializedProperty attenuationCurveOn;
    private SerializedProperty attenuationCurve;

    private GUIContent[] sizesStr = new[]
        {
            new GUIContent("64"), new GUIContent("128"), new GUIContent("256"), new GUIContent("512"),
            new GUIContent("1024"), new GUIContent("2048")
        };

    private int[] sizes = new[] {64, 128, 256, 512, 1024, 2048};

    private GUIContent[] interpolationStepValuesStr = new[]
        {new GUIContent("32"), new GUIContent("16"), new GUIContent("8")};

    private int[] interpolationStepValues = new[] {32, 16, 8};
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (!effect.CheckMinRequirements())
        {
            EditorGUILayout.HelpBox(
                "Render texture support (including RFloat and RGFloat) and shader model 3.0 required.",
        ...
        {
            EditorGUI.BeginChangeCheck();
            attenuationCurve.animationCurveValue = EditorGUILayout.CurveField(new GUIContent("Attenuation curve"),
                                                                              attenuationCurve.animationCurveValue,
                                                                              Color.white,
        ...
        EditorGUILayout.IntPopup(interpolationStep, interpolationStepValuesStr, interpolationStepValues,
                                 new GUIContent("Force sample every"));
